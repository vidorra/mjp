      ...
      <div className="sticky top-0 w-full h-screen flex items-center justify-center">
        <div className="relative w-full h-full max-w-[85vw] 2xl:max-w-[1600px] mx-auto">
          {photos.map((photo, index) => {
            ...
            // Base positions for all photos
            rotation = stackPosition === 0 ? 0 : -5;
            yPosition = stackPosition === 0 ? 0 : 3;
            xPosition = stackPosition === 0 ? 0 : -5;
            scale = stackPosition === 0 ? 1.15 : 1;
            
            const slideProgress = (scrollY * 3) % 1;
            const isSliding = Math.floor(scrollY * photos.length) % photos.length === index;
            
            // Apply transition effect to all photos
            const nextStackPosition = (stackPosition + 1) % photos.length;
            const nextRotation = nextStackPosition === 0 ? 0 : -5;
            const nextYPosition = nextStackPosition === 0 ? 0 : 3;
            const nextXPosition = nextStackPosition === 0 ? 0 : -5;
            const nextScale = nextStackPosition === 0 ? 1.15 : 1;
            
            // Smooth acceleration for synchronized movements
            const acceleratedProgress = Math.min(1, Math.pow(slideProgress * 3, 0.9));
            
            // Always apply transitions, but only when the photo is actively moving
            if (isSliding || stackPosition === photos.length - 1) {
