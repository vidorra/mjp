      ...
      <div className="sticky top-0 w-full h-screen flex items-center justify-center">
        <div className="relative w-full h-full max-w-[85vw] 2xl:max-w-[1600px] mx-auto">
          {photos.map((photo, index) => {
            ...
            if (isSliding) {
              // First half of animation: move to center position
              if (slideProgress < 0.5) {
                const normalizedProgress = slideProgress * 2; // Convert 0-0.5 to 0-1
                rotation = rotation + (0 - rotation) * normalizedProgress;
                yPosition = yPosition + (0 - yPosition) * normalizedProgress;
                xPosition = xPosition + (0 - xPosition) * normalizedProgress;
                scale = scale + (1.15 - scale) * normalizedProgress;
              }
              // Second half: move to next stacked position if not already at center
              else if (stackPosition !== 0) {
                const normalizedProgress = (slideProgress - 0.5) * 2; // Convert 0.5-1 to 0-1
                rotation = 0 + (-8 - 0) * normalizedProgress;
                yPosition = 0 + (5 - 0) * normalizedProgress;
                xPosition = 0 + (-14.6 - 0) * normalizedProgress;
                scale = 1.15 + (1 - 1.15) * normalizedProgress;
              }
            }
