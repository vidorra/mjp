      ...
      <div className="sticky top-0 w-full h-screen flex items-center justify-center">
        <div className="relative w-full h-full max-w-[85vw] 2xl:max-w-[1600px] mx-auto">
          {photos.map((photo, index) => {
            ...
            if (isSliding || isNext) {
              let targetRotation, targetY, targetX, targetScale;
              
              if (isSliding) {
                // Current photo moves to stacked position
                targetRotation = -8;
                targetY = 5;
                targetX = -14.6;
                targetScale = 1;
              } else {
                // Next photo moves to center position
                targetRotation = 0;
                targetY = 0;
                targetX = 0;
                targetScale = 1.15;
              ...
              rotation = rotation + (targetRotation - rotation) * slideProgress;
              yPosition = yPosition + (targetY - yPosition) * slideProgress;
              xPosition = xPosition + (targetX - xPosition) * slideProgress;
              scale = scale + (targetScale - scale) * slideProgress;
            }
