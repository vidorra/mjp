      ...
      <div className="sticky top-0 w-full h-screen flex items-center justify-center">
        <div className="relative w-full h-full max-w-[85vw] 2xl:max-w-[1600px] mx-auto">
          {photos.map((photo, index) => {
            ...
            if (isNext) {
              // Next photo moves to center first
              const progress = slideProgress * 2; // Accelerate the movement to center
              rotation = rotation + (0 - rotation) * progress;
              yPosition = yPosition + (0 - yPosition) * progress;
              xPosition = xPosition + (0 - xPosition) * progress;
              scale = scale + (1.15 - scale) * progress;
            } else if (isSliding && slideProgress > 0.5) {
              // Current photo moves to stacked position after next photo reaches center
              const progress = (slideProgress - 0.5) * 2; // Start halfway through the animation
              rotation = 0 + (-8 - 0) * progress;
              yPosition = 0 + (5 - 0) * progress;
              xPosition = 0 + (-14.6 - 0) * progress;
              scale = 1.15 + (1 - 1.15) * progress;
            }
