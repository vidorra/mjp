  ...
  useEffect(() => {
    const handleScroll = () => {
      if (containerRef.current) {
        ...
        // Calculate scroll progress for consistent speed
        const rawProgress = -rect.top / (containerHeight - window.innerHeight);
        const scrollProgress = Math.max(0, Math.min(1, rawProgress * 1.25));
        setScrollY(scrollProgress);
        
        // Calculate active photo index based on continuous progress
        const baseProgress = scrollProgress * photos.length;
        const newActiveIndex = Math.floor(baseProgress) % photos.length;
      ...
      <div className="sticky top-0 w-full h-screen flex items-center justify-center">
        <div className="relative w-full h-full max-w-[85vw] 2xl:max-w-[1600px] mx-auto">
          {photos.map((photo, index) => {
            ...
            // Calculate continuous slide progress for smoother transitions
            const baseProgress = scrollY * photos.length;
            const currentPhotoIndex = Math.floor(baseProgress);
            const slideProgress = baseProgress - currentPhotoIndex;
